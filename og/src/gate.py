...
from consts import SECURITY_CHECKED_FILE, STAIRS_FILE, STAIRS_CAPACITY, MESSAGES, LOCATIONS
from utils import read_passengers, save_passengers, timestamp, log

def select_optimal_passengers(passengers: list, remaining_capacity: int, remaining_luggage: int) -> list:
    """
    ...
    for passenger in passengers:
        ...
        if len(selected) < remaining_capacity and current_luggage + passenger_luggage <= remaining_luggage:
...
def handle_passengers(queue: Queue):
    ...
    if signal == "airplane_ready":
        ...
        while boarded_passengers < airplane_capacity and passengers:
            ...
            remaining_capacity = min(STAIRS_CAPACITY, airplane_capacity - boarded_passengers)
            remaining_luggage = luggage_limit - total_luggage

            # Wybierz pasażerów, którzy zmieszczą się na schodach i ich bagaże zmieszczą się w samolocie
            batch = select_optimal_passengers(passengers, remaining_capacity, remaining_luggage)

            if not batch:
                break

            batch_luggage = sum(passenger.get('luggageWeight', 0) for passenger in batch)
